...
class Settings:
    ...
    def __init__(self, file_path=None, t_min=None, t_max=None):
        if file_path and t_min and t_max:
            ...
            print('No file path. Exit', file=sys.stderr)
            sys.exit(1)
    ...
    def load_settings(self, file_path):
        [self.size,
         ...
         self.npt,
         self.pt,
         self.print_rate] = np.loadtxt(f'{file_path}/analysis_input.info',
                                       dtype=np.int32)
    ...
    def get_all(self):
        return [self.size, self.replicas,
                self.pt_rate, self.iter,
                self.first, self.npt,
                self.pt, self.print_rate,
                self.tmin, self.tmax]
...
def GetDirectories(path):
    return [d for d in os.listdir(path)
            if os.path.isdir(os.path.join(path, d)) and d != "data"]
